...
class VendingMachine:
    ...
    def is_empty(self):
        if self.inventory == 0:
            if self.funds != 0:
                refunds = self.funds 
                self.funds = 0
                return 'Inventory empty. Restocking required. Here is your ${0}.'.format(refunds)
            else:
    ...
    def vend(self):
        ...
        if  type(isEmpty) != type(0):
            ...
            refunds = self.funds-self.price
            self.funds = 0
            self.inventory-=1
            return  'Here is your {0} and ${1} change.'.format(self.name,refunds)
