...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.digesting:
            print("DEBUG:cout",self.digesting)
            self.digesting -=1
        else:
            pos = self.place
            #print('DEBUG:pos',pos.name)
            
            print('DEBUG:digesting',self.digesting)
            bee = rANTdom_else_none(pos.bees)
            print('DEBUG:bee',bee)
            if bee:
                self.eat_bee(bee)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #print("DEBUG:is_watersafe",insect.is_watersafe)
        if insect.is_watersafe is False :
            #print("DEBUG:water safe")
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        if QueenAnt.the_chosen_one is None:
            QueenAnt.the_chosen_one = self
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        # checking if it's the first one
        if self is not QueenAnt.the_chosen_one:
            print("DEBUG:fake one")
            self.reduce_armor(self.armor)
        else:
            # action
            #print("DEBUG:take action")
            ScubaThrower.action(self, gamestate)
            pos = self.place.exit
            while pos.exit:
                if pos.ant:
                    #print('DEBUG:pos.ant',pos.ant)
                    #print('DEBUG:hasattr',hasattr(pos.ant, 'damage'))
                    if not pos.ant.buffed:
                        print('DEBUG:pos.ant.demage',pos.ant)
                        pos.ant.damage *= 2
                        pos.ant.buffed = True
                    if isinstance(pos.ant, ContainerAnt) is True and pos.ant.contained_ant is not None: 
                        if not pos.ant.contained_ant.buffed:
                            pos.ant.contained_ant.damage *= 2
                            pos.ant.contained_ant.buffed = True
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and QueenAnt.the_chosen_one is self:
            bees_win()
        # END Problem 13
    def remove_from(self, place):
        if QueenAnt.the_chosen_one is self:
            return
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        #print("DEBUG:blocked?",self.place.ant is not None and self.place.ant.blocks_path is True)
...
def make_scare(action, bee):
    ...
    def new_action(gamestate):
        bee.direction = -1
        # remind! this shoud not be bee.action(), which causes inf loops
        action(gamestate)
        bee.direction = 1
